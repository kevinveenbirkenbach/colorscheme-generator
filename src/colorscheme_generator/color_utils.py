...
def generate_full_palette(base_color, count=7, shades=100):
    """
    Generate a full palette of HEX and RGB colors.

    For each of the `count` base colors (generated by hue rotation),
    generate `shades` variations by changing lightness.

    Returns:
        dict: Mapping of CSS variable names to HEX/RGB values
    """
    full_palette = {}
    ...
    for color_index, color in enumerate(base_colors):
        for shade in range(shades):
            # Lightness 0.0 -> 1.0 linear mapping
            target_lightness = shade / (shades - 1)

            hex_value = adjust_color(color, target_lightness=target_lightness)
            rgb_value = adjust_color_rgb(color, target_lightness=target_lightness)

            full_palette[f'--color-{color_index+1}-{shade:02d}'] = hex_value
            full_palette[f'--color-rgb-{color_index+1}-{shade:02d}'] = rgb_value
